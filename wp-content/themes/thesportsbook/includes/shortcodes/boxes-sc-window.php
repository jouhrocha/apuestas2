 ...
 <div id="fly_boxes" class="flytonic_sc_editor" title="Alert Boxes Shortcode"  style="display:none">
	...
	<div>
    	...
    	<select class="fly_textinput" name="box_style" id="box_style">
    	     	<option value="info"><?php _e('Info Box', 'thesportsbook'); ?></option>
		<option value="check"><?php _e('Check Box', 'thesportsbook'); ?></option>
               	 <option value="download"><?php _e('Download Box', 'thesportsbook'); ?></option>
		<option value="error"><?php _e('Error Box', 'thesportsbook'); ?></option>
   		<option value="alert"><?php _e('Alert Box', 'thesportsbook'); ?></option>
    	</select>

	</div>

	<div>
    	<label><?php _e('Alert Box Text', 'thesportsbook'); ?></label>
	<textarea class="fly_textinput" id="box_body" style="width: 100%; height: 80px;"></textarea>
	<p><?php _e('Enter text here or within the post itself', 'thesportsbook'); ?></p>
   	</div> 
		...
		jQuery('#fly_boxes').dialog({
			...
			buttons: {
				Ok: function() {
					...
					if (Editor == null) {
						jQuery('#fly_boxes').data('Editor').insertContent(str);
					} else {
						Editor.focus();
