...
add_action('init', 'banner_redirect'); 
...
function banner_redirect() {
	...
	if (isset($_GET['dlkey'])) {
		$request = '/outgoing/'.$_GET['dlkey'].'/';
	}
	

	if ( strpos('/'.$request, '/outgoing/') ) {
		$dl_key = explode('outgoing/', $request);
		$dl_key = $dl_key[1];
...
query_posts('post_type=banner&meta_key=_fly_bannerslug&meta_value='.$dl_key .''); 
if ( have_posts() ) : the_post();

$url= get_post_meta($post->ID, '_fly_bannerurl', true);
...
{
	add_submenu_page('design-options', __('Redirect Options', 'thesportsbook'), __('Redirect Options', 'thesportsbook'), 'update_themes', 'redirect-options', 'redirect_show_ui');
...
<style>
	...
	<div class="wrap meta-box-sortables">
        ...
        <h2><?php _e('Custom Redirects', 'thesportsbook'); ?></h2>
        ...
        <div id="poststuff">
	        ...
	        <table class="redirects_table widefat" width=620>
            	<thead>
                	<tr>
                    	<th width=120><?php _e('Redirect Key', 'thesportsbook'); ?></th>
                        <th width=400><?php _e('Url', 'thesportsbook'); ?></th>
                        <th width=75><?php _e('Hits', 'thesportsbook'); ?></th>
                        <th width=160><?php _e('Actions', 'thesportsbook'); ?></th>
					</tr>
				</thead>
                <tbody>
                ...
                <?php if (count($redirects) == 0) : ?>
                	<tr>
                    	<td colspan="4"><em><?php _e('No custom redirects', 'thesportsbook'); ?></em></td>
					</tr>
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td>
                        	<span><a href="#" onclick="editRow(this)"><?php _e('Edit', 'thesportsbook'); ?></a> | </span>
                        	<a href="?page=redirect-options&action=delete-redirect&key=<?php echo $redirect['key'];?>"><?php _e('Delete', 'thesportsbook'); ?></a> | 
                        	<a href="?page=redirect-options&action=reset-redirect&key=<?php echo $redirect['key'];?>"><?php _e('Reset Hits', 'thesportsbook'); ?></a></td>
                    </tr>
                    <?php endforeach; ?>
            ...
            <br />
            <input class="button-secondary" type="submit" name="action" value="<?php _e('Save Changes', 'thesportsbook'); ?>" />
		</form>
        <script>
        ...
        <br />
        <h3><?php _e('Create a custom redirect', 'thesportsbook'); ?></h3>
...
?>
        ...
        <form method="post" action="?page=redirect-options">
            <label><?php _e('Key', 'thesportsbook'); ?>:</label>
            <input type="text" name="key" value="<?php echo $mykey;?>" />
            <label><?php _e('URL', 'thesportsbook'); ?>:</label>
            <input style="width: 200px" name="url" value="<?php echo $myurl;?>" type="text" />
            <input class="button-primary" type="submit" name="action" value="<?php _e('Create Redirect', 'thesportsbook'); ?>" />
		</form>
		</div><!--poststuff-->
